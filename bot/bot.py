...
from bot.api import LocalAPIClient, WarframeStatusAPIClient
...
class Ordis(Bot):
    def __init__(self) -> None:
        ...
        self.database_items_sync_task: Task[None] | None = None

    async def get_context(self, message: Message, *, cls: Context = Context) -> Context:  # type: ignore [assignment]
        return await super().get_context(message, cls=cls)  # type: ignore [assignment]
    ...
    async def start(self, *args, **kwargs) -> None:  # pyright: ignore[reportMissingParameterType, reportUnknownParameterType]
    ...
    async def close(self) -> None:
        if self.database_items_sync_task is not None:
